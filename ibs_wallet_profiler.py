...
def profile_wallet(web3: Web3, rpc_url: str, address: str, ibs_is_token0: bool, ibs_decimals: int, usdt_decimals: int, current_price: Decimal) -> dict[str, Any]:
    ...
    category, reasons, arbitrage_assessment = classify(address, is_contract, trades, protocol_source_raw, external_in_raw, first_buy)
...
def main() -> None:
    ...
    for item in sellers:
        ...
        if existing is None:
            pending.append({"event_id": item["event_id"], "event_ids": [item["event_id"]], "address": item["address"], "tx_hash": trade.tx_hash, "block_number": trade.block_number, "log_index": trade.log_index, "ibs_raw": str(trade.ibs_raw), "usdt_raw": str(trade.usdt_raw)})
        else:
            ...
            if (trade.block_number, trade.log_index) > (int(existing["block_number"]), int(existing["log_index"])):
                existing.update({"event_id": item["event_id"], "tx_hash": trade.tx_hash, "block_number": trade.block_number, "log_index": trade.log_index, "ibs_raw": str(trade.ibs_raw), "usdt_raw": str(trade.usdt_raw)})
...
def build_message(profile: dict[str, Any], trigger: Trade, ibs_decimals: int, usdt_decimals: int, lp_ibs_raw: int, lp_usdt_raw: int) -> str:
    ...
    return "\n".join([
        ...
        f"其他协议地址转入：{amount(profile['protocol_in_raw'], ibs_decimals):,.4f} IBS", "",
        f'<a href="https://bscscan.com/address/{address}">查看地址</a> ｜ <a href="https://bscscan.com/tx/{trigger.tx_hash}">查看触发交易</a>',
        "说明：只有本池买入→卖出的可匹配部分计算盈亏；协议返还、奖励、转账和其他池来源不再冒充利润。分类是链上行为标签，不是身份或违法定性。",
    ])
